...
export default function MessagesPage() {
  ...
  const handleSelectConversation = async (conversation) => {
    ...
    const msgs = await messageController.getMessages(conversation.id);
    setMessages(msgs);
  };
  ...
  const handleSendMessage = async (e) => {
    ...
    const { success, message } = await messageController.sendMessage(selectedConversation.id, newMessage);
    if (success) {
      ...
      setConversations(prev => prev.map(c => 
        c.id === selectedConversation.id 
          ? { ...c, lastMessage: message.text, timestamp: "Just now" } 
          : c
      ));
    }
  };
  ...
  return (
    <DashboardLayout>
      <div className="flex h-[calc(100vh-64px)] bg-background-light dark:bg-background-dark overflow-hidden">
        {/* Conversation List */}
        <div className={`w-full md:w-80 border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-[#192430] flex flex-col ${selectedConversation ? 'hidden md:flex' : 'flex'}`}>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-baseline mb-1">
                        ...
                        <span className="text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap ml-2">{conv.timestamp}</span>
                    </div>
                    <p className={`text-sm truncate ${conv.unread > 0 ? 'font-bold text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400'}`}>
                        {conv.lastMessage}
                    </p>
                </div>
              </div>
            ...
            {selectedConversation ? (
                <>
                    ...
                    <div className="p-4 bg-white dark:bg-[#192430] border-b border-gray-200 dark:border-gray-700 flex items-center gap-3 shadow-sm z-10">
                        ...
                        <img src={selectedConversation.participantAvatar} alt={selectedConversation.participantName} className="w-10 h-10 rounded-full object-cover" />
                        <div>
                            <h3 className="font-bold text-gray-900 dark:text-white">{selectedConversation.participantName}</h3>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4">
                        {messages.map((msg) => {
                            const isMe = msg.senderId === 1; // Assuming current user ID is 1
                            return (
